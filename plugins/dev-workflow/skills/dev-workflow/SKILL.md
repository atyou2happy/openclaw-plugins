...
3. 用户确认前 → **只允许只读操作**
4. 确认后 → 解锁写权限
5. **Standard/Full 模式：code_graph impact 分析** ⭐ — 调用 `code_graph` tool 的 `impact` action，生成修改影响清单，防止遗漏关联文件。详见 `references/code-analysis-tools.md`
...
**code_graph 完整性验证**（Standard/Full）：调用 `code_graph` tool 的 `verify` action，验证所有关联节点已被修改覆盖，防止遗漏。详见 `references/code-analysis-tools.md`
...
**code_graph 调用链追踪**（Standard/Full）：调用 `code_graph` tool 的 `trace` action，追踪从 Bug 表现模块到可能的根因模块之间的调用链，辅助定位根因。详见 `references/code-analysis-tools.md`
